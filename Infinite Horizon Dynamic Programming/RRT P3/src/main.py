...
class RRT_Star():
    ...
    def __Near(self, Xnew, beta = 2): 
        ...
        Xnear = []
        Xnear = [vertex for vertex in self.vertices if self.__dist(Xnew, vertex) < radius]
        return Xnear
    ...
    def Extend(self):
        ...
        if self.__collision_checker(Xnearest, Xnew) == False:
            ...
            #no collision
            # Xnearest may not be the best connection to Xnew, we have to find xmin
            ...
            if Xnew == self.terminal:
                ...
                #after we find connentcion between xmin and Xnew;
                #we have to know can this Xnew be parents for parent
                #print(Xnear)

                myXnear = Xnear.remove(xmin)
                if myXnear == None:
                    myXnear = []
                ...
                for xnear in myXnear:
                    if self.__collision_checker(Xnew, xnear) == False and\
                     ...
                     #the old path to xnear is large than the path from xnew to x near
                     #therefore the path has to change, we have to find xnear parent
    ...
    def sample(self):
        self.Xrand = (random.uniform(-2,12),random.uniform(-2,15))
...
if __name__ == "__main__":
    ...
    with open (path + '/data/' + myfiles[1], 'r') as file_obj:
