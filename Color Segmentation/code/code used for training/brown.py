...
Brown_Class = [[], [], []] # initialize the red class as a list which contains three sub list for RGB
#Red_Class_area = []
...
for img_file in G_P:
    ...
    Brown_Class[0].extend(R_img.tolist()) #append final data R data list in red class(list) at first space
    Brown_Class[1].extend(G_img.tolist()) #append final data G data list in red class(list) at second space
    Brown_Class[2].extend(B_img.tolist()) #append final data B data list in red class(list) at third space
...
output_brown = open('brown.pkl','wb') # this file save a list for red class, which includes R G B value for that
pickle.dump (Brown_Class, output_brown)
output_brown.close()
