...
notsoRed_Class = [[], [], []] # initialize the red class as a list which contains three sub list for RGB
#Red_Class_area = []
...
for img_file in G_P:
    ...
    notsoRed_Class[0].extend(R_img.tolist()) #append final data R data list in red class(list) at first space
    notsoRed_Class[1].extend(G_img.tolist()) #append final data G data list in red class(list) at second space
    notsoRed_Class[2].extend(B_img.tolist()) #append final data B data list in red class(list) at third space
...
output_notsored = open('notredbarrel.pkl','wb') # this file save a list for red class, which includes R G B value for that
pickle.dump (notsoRed_Class, output_notsored)
output_notsored.close()

#output_notsoredarea = open('notsoredarea.pkl','wb')
#pickle.dump (notsoRed_Class_area, output_notsoredarea)
#output_notsoredarea.close()
